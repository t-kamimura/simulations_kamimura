% fileName: thesis_rateEnergy.m
% initDate:　2020/12/04
% Object: (脚バネに蓄えられる最大エネルギー)/(総エネ)で比率を表すプログラム
% 修正1 : 2021/1/26 二次元プロットでpdf保存してくれる 
% 修正2 : 2021/2/12 卒論発表用に安定で記号大→運動一致で記号大
...
for i = 1: length(fixedPoint)
    ...
    if logdata(i).eeout(3) ==1                                                                      
        ...
        if fixedPoint(i).u_fix(3) > 0  &&  fixedPoint(i).trajectory.qout(i_middle, 4) > 0          
            ...
            if fixedPoint(i).q_constants(3) > 0                                                           
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, '^','markerfacecolor', '#0072BD', 'markeredgecolor', '#0072BD')  
                    hold on    

            %前脚から接地
            elseif fixedPoint(i).q_constants(3) < 0                           
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, '^', 'markerfacecolor', '#4DBEEE', 'markeredgecolor', '#4DBEEE')    
                    hold on  
            end
        ...
        elseif fixedPoint(i).u_fix(3)  < 0  && fixedPoint(i).trajectory.qout(i_middle, 4) < 0      
            ...
            %後脚から接地
            if fixedPoint(i).q_constants(3) > 0    
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, '^', 'markerfacecolor', '#D95319', 'markeredgecolor', '#D95319')      
                    hold on

            %前脚から接地
            elseif fixedPoint(i).q_constants(3) < 0   
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, '^','markerfacecolor', '#EDB120', 'markeredgecolor', '#EDB120')      
                    hold on
        ...
        elseif (fixedPoint(i).u_fix(3)) > 0 && (fixedPoint(i).trajectory.qout(i_middle, 4)) < 0            
            ...
            %後脚から接地
            if fixedPoint(i).q_constants(3) > 0    
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'o','markerfacecolor', '#0072BD', 'markeredgecolor', '#0072BD')      
                    hold on

            %前脚から接地
            elseif fixedPoint(i).q_constants(3) < 0   
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'o', 'LineWidth', 1.0, 'markerfacecolor', '#4DBEEE', 'markeredgecolor', 'k', 'MarkerSize',12)     
                    hold on 
            end 
        ...
        elseif (fixedPoint(i).u_fix(3)) < 0 && (fixedPoint(i).trajectory.qout(i_middle, 4)) > 0             
            ...
            if fixedPoint(i).q_constants(3) > 0   
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'o', 'LineWidth', 1.0, 'markerfacecolor', '#D95319', 'markeredgecolor', 'k', 'MarkerSize',12)      
                    hold on

            elseif fixedPoint(i).q_constants(3) < 0   
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'o','markerfacecolor', '#EDB120', 'markeredgecolor', '#EDB120')      
                    hold on
            end 
    ...
    elseif  logdata(i).eeout(3) == 3
        ...
        if fixedPoint(i).u_fix(3) > 0 
            ...
            %後脚から
            if fixedPoint(i).q_constants(3) > 0                                                           
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 's','markerfacecolor', '#0072BD', 'markeredgecolor', '#0072BD')     
                    hold on

            %前脚から
            elseif fixedPoint(i).q_constants(3) < 0                          
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 's','markerfacecolor', '#4DBEEE', 'markeredgecolor', '#4DBEEE')   
                    hold on
            
            %プロンク
            elseif fixedPoint(i).q_constants(3) == 0
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'd','markerfacecolor', '#0072BD', 'markeredgecolor', '#0072BD')      
                    hold on
            end
        ...
        elseif fixedPoint(i).u_fix(3)  < 0 
            ...
            %後脚から
            if fixedPoint(i).q_constants(3) > 0    
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 's','markerfacecolor', '#D95319', 'markeredgecolor', '#D95319')      
                    hold on

            %前脚から
             elseif fixedPoint(i).q_constants(3) < 0   
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 's','markerfacecolor', '#EDB120', 'markeredgecolor', '#EDB120')  
                    hold on
 
            %プロンク
            elseif fixedPoint(i).q_constants(3) == 0
                    plot(rad2deg(fixedPoint(i).q_constants(3)), logdata(i).rate, 'd','markerfacecolor', '#D95319', 'markeredgecolor', '#D95319')    
                    hold on
